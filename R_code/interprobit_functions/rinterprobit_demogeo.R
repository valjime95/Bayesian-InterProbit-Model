...
newrho=abs(rnorm(1,mean=oldrho,sd=1))
...
if (Type =='demo_geo'){
  ...
  # newtau = abs(oldtau + rnorm(1,mean=0,sd=.1)) #*
  
  newtau=abs(rnorm(1,mean=oldtau,sd=1))
  newphi = exp(newtau)/(exp(newtau)+1) #*
  W_n = (newphi*W_zipcode) + ((1-newphi)*W_dem) #*
